...
export default class FluidAnimation {
  constructor(opts) {
    ...
    this._pointers[0].color = colors[Math.floor(Math.random() * colors.length)];
    this._splatStack = [];
  ...
  onMouseMove = (e) => {
    ...
    if (Date.now() - this._colorTimer > 1000) {
      this._colorTimer = Date.now();
